...
typedef struct _GAMESTATE {
  int    ShotNum;    // number of Shot // 第几次投递
  ...
  double  body[16][2];  // body[n][0] : x of coordinate of n th stone // stone冰壶的坐标
              // body[n][1] : y of coordinate of n th stone
              // 每队8个，共16个冰壶

}GAMESTATE, * PGAMESTATE;

typedef struct _ShotInfo //每次击打的信息：速度
{
...
typedef struct _MOTIONINFO // 运动的信息
{
  float x_coordinate;  // x的位置
  float y_coordinate;  // y的位置
  float x_velocity;    // x轴的速度
  float y_velocity;   // y轴的速度
...
static const float HOUSE_R = (float)1.870;  // radius of house 大本营半径 应该改为1.83
...
float distFromCenter(double x, double y){
...
int main() {
  Init();
  getBestShot();
  std::cout << "shot.speed=" << shot.speed
      << "\nshot.h_x=" << shot.h_x
      << "\nshot.angle=" << shot.angle << std::endl;
